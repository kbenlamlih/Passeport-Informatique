...
class SudokuSolver:
    ...
    def __init__(self, grid):
        ...
        self.dico=dict()
        self.grid=grid
        self.valeurs_possibles= [n for n in range(1,10)]
        for element in self.grid.get_empty_pos():
            self.dico[element]=self.valeurs_possibles.copy()
    ...
    def reduce_all_domains(self):
        ...
        *Indication: Vous pouvez utiliser les fonction ``get_row``, ``get_col`` et ``get_region`` de la grille*
        ...
        for element in self.dico:
            remove = self.grid.get_row(element[0])
            remove = remove + self.grid.get_col(element[1])
            remove = remove + self.grid.get_region(element[0] // 3, element[1] // 3)

            for el in remove:
                if el in self.dico[element]:
                    self.dico[element].remove(el)
    ...
    def reduce_domains(self, last_i, last_j, last_v):
        ...
        :type last_v: int
        """
        # Mettre à jour la liste des valeurs possibles pour la case ou on a ecrit:
        del self.dico[last_i, last_j]

        # Elimine les possibilités pour les cases de la même colonne, de la même ligne et de la même région
        for i in range(9):
            if (i, last_j) in self.dico.keys() and last_v in self.dico[i, last_j]:
                self.dico[i, last_j].remove(last_v)

            if (last_i, i) in self.dico.keys() and last_v in self.dico[last_i, i]:
                self.dico[last_i, i].remove(last_v)

            if (3 * (last_i // 3) + (i % 3), 3 * (last_j // 3) + (i // 3)) in self.dico.keys() and last_v in self.dico[
                (3 * (last_i // 3) + (i % 3)), (3 * (last_j // 3) + (i // 3))]:
                self.dico[(3 * (last_i // 3) + (i % 3)), (3 * (last_j // 3) + (i // 3))].remove(last_v)
    ...
    def commit_one_var(self):
        ...
        """
        for elt in self.dico:
            if len(self.dico[elt]) == 1:
                self.grid.write(elt[0], elt[1], self.dico[elt][0])
                return elt[0], elt[1], self.dico[elt][0]
        return None
    ...
    def solve_step(self):
        ...
        *Variante avancée: en plus de vérifier s'il ne reste plus qu'une seule possibilité pour une case,
        ...
        while len(self.grid.get_empty_pos()) > 0:
            ...
            valeurs = self.commit_one_var()
            self.reduce_all_domains()
            if valeurs != None:
                self.reduce_domains(*valeurs)
            else:
                break
    ...
    def branch(self):
        ...
        *Variante avancée: Un choix judicieux de variable libre,
        ...
        # on cherche l'une des cases avec le moins de valeurs possibles
        minimum = 9
        position_minimum = (0, 0)
        for element in self.dico:
            if len(self.dico[element]) < minimum:
                minimum = len(self.dico[element])
                position_minimum = element
            if minimum == 2:# on ne peut pas trouver de cas plus favorable
                break

        # pour chaque valeurs possibles de cette case on créer une copie de la grid et un sudokuSolver associé

        for element in self.dico[position_minimum]:
            new_grid = self.grid.copy()
            new_grid.write(*position_minimum, element)
            sous_solvers.append(SudokuSolver(new_grid))
    ...
    def solve(self):
        ...
        # on commence par tenter de résoudre avec solve_step
        self.solve_step()
        ...
        sous_solvers = self.branch()

        for sous_solver in sous_solvers:

            rep = sous_solver.solve()
